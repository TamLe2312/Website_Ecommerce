        ...
        <main class="main">
            ...
            <div id="product-container" class="container">
                <div class="products-container" data-layout="fitRows">
                    <?
                    if (isset($connection) && isset($_GET['Search'])) {
                        $message = "";
                        $Search = $_GET['Search'];
                        $statement = $connection->prepare("SELECT * FROM products WHERE productName LIKE '%$Search%'");
                        $statement->execute([]);
                        $statement->setFetchMode(PDO::FETCH_ASSOC);
                        $ProductList = $statement->fetchAll();
                        $Check = $statement->RowCount();
                        if ($Check < 1) {
                            $message = "Sorry,We don't have that product.";
                        }
                    } ?>
                    <?
