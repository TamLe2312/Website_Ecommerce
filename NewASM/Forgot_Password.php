...
function send_password_reset($get_username, $get_email, $token)
{
    ...
    // Mail body content 
    $bodyContent = "<h1>Hello,$get_username</h1>";
    $bodyContent .= '<p>We received a message that you send a reset password notify</p>';
    $bodyContent .= '<p>This is your reset password link,please click link below to start reset your password</p><br/><br/>';
    $bodyContent .= "<a href ='http://localhost/newasm/password-change.php?username=$get_username&tokenchangepassword=$token'>Click Here</a>";
...
if (isset($_POST['password_reset_link'])) {
    if (isset($_POST['email']) && isset($_POST['username'])) {
        ...
        if (isset($connection) && count($errors) < 1) {
            try {
                $token = md5(rand());
                $check_email = "SELECT username,email FROM users WHERE username = :username";
                $statement = $connection->prepare($check_email);
                $statement->execute([
                    ':username' => $username
                ]);
                $result = $statement->setFetchMode(PDO::FETCH_ASSOC);
                $result1 = $statement->fetchAll();
                $count_user = count($result1);
                if ($count_user > 0) {
                    $get_username = $result1[0]['username'];
                    $get_email = $result1[0]['email'];
                    $update_token = "UPDATE users SET verify_token='$token' WHERE username = :username";
                    $statement1 = $connection->prepare($update_token);
                            ...
                            <div class="tab-content">
                                <div class="tab-pane fade show active" id="register-2" role="tabpanel" aria-labelledby="register-tab-2">
                                    ...
                                    <form class="user" action="./Forgot_Password.php" method="POST">
                                        ...
                                        <button class="btn btn-primary btn-user btn-block" name="password_reset_link">
                                            <a class="alink">
                                                Reset Password
                                            </a>
                                        </button>
